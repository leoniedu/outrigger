...
habilidade = [1.02, 1.0, 1, 1, 1, 1.02, .98, 1.01, 1.02]
...
```{python}
seat_entry_weights = [1, 4, 4, 4, 4, 1]
...
## Análise de Trim (Balanço)

O modelo considera o balanço fore-aft (proa-popa) da canoa baseado no peso dos remadores.
Momento de trim positivo = canoa pesada na popa. Momento de trim negativo = canoa pesada na proa.

```{python}
#| label: tbl-trim
#| tbl-cap: "Análise de trim por turno do ciclo"
...
if trim_stats:
    ...
    print(f"\nMomentos por turno do ciclo:")
    for t, m in enumerate(trim_stats['trim_moments']):
        direcao = "popa" if m > 0 else "proa" if m < 0 else "neutro"
        print(f"  Turno {t+1}: {m:+.1f} kg-m ({direcao})")
else:
    print("Trim não calculado (trim_penalty_weight = 0)")
```
