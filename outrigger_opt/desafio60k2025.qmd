...
print("Tripulação e habilidades:")
for nome, hab in zip(paddlers['name'], habilidade):
    #emoji = "💪" if hab > 1.0 else ("😴" if hab < 1.0 else "")
    print(f"  {nome}: {hab:.0%}")
```
...
for i in range(9):
    for j in range(6):
        text = '✓' if eligibility[i, j] == 1 else ''
...
*Justificativa*: O atleta vai subir pro banco 1 provavelmente frio, dificilmente acertará o ritmo de primeira. Melhor entrar no 2, esquentar e ir pro 1.

**Popa**: Marcelo entra no banco 5, depois vai pro 5. Zé direto no 6.
:::

### Pesos de Entrada (`seat_entry_weights`)

Os pesos refletem a dificuldade/preferência de entrada em cada banco:

| Banco | Peso | Justificativa |
|-------|------|---------------|
| 1 (Voga) | 1.0 | Fácil de subir, mas prefere-se entrar aquecido |
| 2 | 2.0 | Banco preferido para entrada — esquenta antes de ir pro 1 |
| 3 | 1.2 | Ricardo é o nosso Hip principal  |
| 4 | 1.2 | Meio — sem grande diferença |
| 5 | 1.2 | Entrada padrão da popa |
| 6 (Leme) | 1.0 | Alguns têm dificuldade de subir |
...
Achava que poderia ser tipo 30 min remando, 15 min descansando. Mas tem que ser **curto mesmo** — 10 a 13 minutos. Se não cansa de mais. Com mais trocas, a troca é "traumática" para alguns de nós. 
:::
...
summary_display.columns = ['turno (min)', 'Nº turnos', 'Output Médio', 'Tempo (min)', 'Tempo (horas)']
...
```{python}
#| label: fig-heatmap
#| fig-cap: "Mapa de calor do cronograma de rotação"
...
ax.set_title('Cronograma de Rotação - Desafio 60K 2025', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.show()
```
...
| Duração do stint | `{python} best_stint` min |
| Número de stints | `{python} n_stints` |
